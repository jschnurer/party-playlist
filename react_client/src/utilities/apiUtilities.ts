...
interface IResponseErrorMessageDictionary {
  [statusCode: number]: string | undefined,
}

export async function throwIfResponseError(response: Response, messageMapper?: IResponseErrorMessageDictionary) {
  const customMessage = messageMapper?.[response.status];

  if (response.status === 500) {
    await throwHttp500Error(response, customMessage || "The server encountered a fatal error.");
  } else if (response.status === 400) {
    await throwResponseError(response, customMessage || `The request sent to the server was invalid.`);
  } else if (response.status === 401) {
    await throwResponseError(response, customMessage || "The requested resource requires authentication to access.");
  } else if (response.status === 403) {
    await throwResponseError(response, customMessage || "You do not have permission to access the requested resource.");
  } else if (response.status === 404) {
    await throwResponseError(response, customMessage || "The requested resource was not found.");
  } else if (response.status < 200 || response.status > 299) {
    await throwResponseError(response, customMessage || `Server returned ${response.statusText}.`);
  }
}
